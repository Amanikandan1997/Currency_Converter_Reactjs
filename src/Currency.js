...
function Currency() {
    ...
    const[xchagerate,setxchangerate]=useState(null)
    useEffect(()=>{
        const changerate = async ()=>{
            try{
                ...
                const response=await axios.get(url);
                 ...
                 setxchangerate(response.data.rates[tocurrency]);
      ...
      useEffect( () => {
           if(xchagerate !== null ){
            setconvertamount( amount * xchagerate );
           }
        },[amount,xchagerate]);
  ...
  return (
    ...
    <option value="ZWB" label="Zimbabwean bonds">ZWB</option>
            ...
            <p>Design by <a href=''>Manikandan U K I</a></p>
            </div>
         </div>
        </div>
